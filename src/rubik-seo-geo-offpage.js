...
/* Personal-data minimisation for excerpts and AI evidence: e-mails, phone numbers, URL
   user-info/query strings and key=value credentials are removed. */
function minimize(value,max=280){
  return text(value)
    ...
    .replace(/(?:\+\d[\d\s().-]{7,}\d)|(?:\b\d{3}[\s.-]?\d{3}[\s.-]?\d{3,4}\b)/g,'[phone]')
    .slice(0,max);
}
...
/* One dimension measurement. Accepts a CORE-7 result envelope (status, provider,
   ...
   capturedAt, coverage, rows}. Only a live provider envelope can be VERIFIED. Without a
   usable status there are no rows and no counts (never zero). */
function measurement(input){
  const v=input&&typeof input==='object'?input:{};
  const envelope=!!(v.provenance&&typeof v.provenance==='object'&&'connection' in v);
  ...
  return {
    ...
    method:envelope?(v.provenance.method==='api'?'api':'mock'):(SOURCE_METHODS.includes(v.method)&&v.method!=='api'?v.method:'manual'),
    verified:envelope&&v.connection==='VERIFIED',
    capturedAt,coverage,
...
function snapshot(input={},{providers,releaseE,core,config}={}){
  ...
  const register=(dim,m,extra={})=>{m.usable=USABLE.includes(m.status)&&!!m.capturedAt;const ref=id+':'+dim;if(m.usable)evidence.push({id:ref,dimension:dim,provider:m.provider,method:m.method,verified:m.verified,capturedAt:m.capturedAt,coverage:m.coverage,...extra});return ref;};
  ...
  if(v.backlinks){
    ...
    const m=measurement(v.backlinks),n=providers.normalizeBacklinks(m.rows,{measuredAt:m.capturedAt,provider:m.provider});
    ...
    out.dimensions.backlinks={status:m.status,provider:m.provider,method:m.method,verified:m.verified,capturedAt:m.capturedAt,coverage:m.coverage,rejectedRows:n.rejected,rows:n.rows,
  ...
  if(v.mentions){
    const m=measurement(v.mentions),records=[],rejected=[];
    ...
    out.dimensions.mentions={status:m.status,provider:m.provider,method:m.method,verified:m.verified,capturedAt:m.capturedAt,coverage:m.coverage,rejectedRows:rejected.length,records,
  ...
  if(v.localCitations){
    const m=measurement(v.localCitations),c=m.status==='NOT_MEASURED'&&!m.rows.length?{status:'NOT_MEASURED',listings:[]}:citationConsistency(m.rows,{core,config});
  ...
  if(v.referrals){
    const m=measurement(v.referrals),rows=m.rows.map(r=>({source:fold(r?.source),medium:fold(r?.medium),sessions:Number.isFinite(Number(r?.sessions))&&r?.sessions!==''&&r?.sessions!=null?Number(r.sessions):null,landingPage:cleanUrl(r?.landingPage)||null})).filter(r=>r.source&&r.sessions!=null);
...
/* geoRun(input, {querySet, profile}): one run of one query on one engine. Scraping or any
   undeclared method is rejected (no scraping against terms of service). No answer shown is
   its own outcome (NO_ANSWER), never a zero. Manual observations are never "verified". */
function geoRun(input,{querySet:qs,profile:p}={}){
  ...
  if(v.status==='ERROR')return {ok:true,run:{querySetHash:qs.hash,queryId:q.id,engine:text(v.engine),surface:v.surface==='consumer-ui'?'consumer-ui':'api',model:text(v.model)||null,locale:text(v.locale)||q.locale,market:text(v.market).toUpperCase()||q.market,runAt,runIndex:Number.isInteger(v.runIndex)?v.runIndex:null,method:v.method,verified:false,outcome:'ERROR',mentioned:null,detection:'NONE',controlsMentioned:[],citations:[],citationsCaptured:false,cited:null,answerExcerpt:null}};
  ...
  return {ok:true,run:{
    querySetHash:qs.hash,queryId:q.id,engine:text(v.engine),surface:v.surface==='consumer-ui'?'consumer-ui':'api',model:text(v.model)||null,
    locale:text(v.locale)||q.locale,market:text(v.market).toUpperCase()||q.market,runAt,runIndex:Number.isInteger(v.runIndex)?v.runIndex:null,method:v.method,
    verified:v.method==='api'&&v.connection==='VERIFIED',
...
/* summarizeGeo(runs, {querySet}): distribution, range and variability per engine/locale/
   market. Observational metrics only (not referral traffic, not business outcome). */
function summarizeGeo(runs,{querySet:qs,minRunsPerQuery=3}={}){
  if(!qs?.hash)throw new TypeError('querySet must be injected');
  const valid=arr(runs).filter(r=>r&&r.querySetHash===qs.hash);
  const base={querySet:{id:qs.id,version:qs.version,hash:qs.hash},limits:[SAMPLE_LIMIT,'Métrica observacional: no es tráfico de referencia ni resultado de negocio.','Las ejecuciones de una misma consulta están correlacionadas; el intervalo es orientativo.']};
  if(!valid.length)return freeze({...base,status:'NOT_MEASURED',groups:[],methods:[],window:null});
  const groups=new Map();
  for(const r of valid){const k=[r.engine,r.locale,r.market].join('|');if(!groups.has(k))groups.set(k,[]);groups.get(k).push(r);}
  const out=[];
  for(const [k,rs] of [...groups.entries()].sort(([a],[b])=>a<b?-1:1)){
    const [engine,locale,market]=k.split('|');
    ...
    const perQuery=qs.queries.filter(q=>q.locale===locale||!locale).map(q=>{const qr=answered.filter(r=>r.queryId===q.id);const qk=qr.filter(r=>r.mentioned!==null);const m=qk.filter(r=>r.mentioned).length;return {queryId:q.id,runs:rs.filter(r=>r.queryId===q.id).length,answered:qr.length,mentioned:m,mentionRate:qk.length?+(m/qk.length).toFixed(4):null,consistent:qk.length<2?null:(m===0||m===qk.length)};});
    const withRuns=perQuery.filter(q=>q.runs>0),rates=perQuery.map(q=>q.mentionRate).filter(x=>x!=null);
    const positions=answered.flatMap(r=>r.citations.filter(c=>c.own&&c.position!=null).map(c=>c.position)).sort((a,b)=>a-b);
    const minRuns=withRuns.length?Math.min(...withRuns.map(q=>q.runs)):0,coverage=+(withRuns.length/qs.queries.length).toFixed(4);
    const controls=answered.reduce((a,r)=>a+r.controlsMentioned.length,0);
    const models=uniq(rs.map(r=>r.model).filter(Boolean)).sort();
    const cons=perQuery.filter(q=>q.consistent!==null);
    const flags=[];if(minRuns<minRunsPerQuery)flags.push('FEW_RUNS_PER_QUERY');if(coverage<1)flags.push('INCOMPLETE_QUERY_COVERAGE');if(controls)flags.push('HALLUCINATED_CONTROL_MENTIONS');if(models.length>1)flags.push('MODEL_CHANGED_WITHIN_WINDOW');if(rs.some(r=>r.method==='manual'))flags.push('MANUAL_OBSERVATIONS_UNVERIFIED');
    out.push({engine,locale,market,models,surfaces:uniq(rs.map(r=>r.surface)).sort(),runs:rs.length,answered:answered.length,noAnswer,errors,queryCoverage:coverage,minRunsPerQuery:minRuns,
      ...
      perQuery,confidence:flags.some(f=>['FEW_RUNS_PER_QUERY','INCOMPLETE_QUERY_COVERAGE','HALLUCINATED_CONTROL_MENTIONS'].includes(f))||answered.length<10?'low':'medium',flags});
  }
...
/* compareGeo(previous, current): a change is reported only when the query set is the same
   and the 95% intervals do not overlap; otherwise WITHIN_NOISE or NOT_COMPARABLE. */
function compareGeo(a,b){
  ...
  const groups=b.groups.map(g=>{
    const p=a.groups.find(x=>x.engine===g.engine&&x.locale===g.locale&&x.market===g.market);
    if(!p)return {engine:g.engine,locale:g.locale,market:g.market,change:'NOT_COMPARABLE',reason:'NO_PREVIOUS_GROUP'};
    const x=p.mentionRate.ci95,y=g.mentionRate.ci95;
    const change=!x||!y?'NOT_COMPARABLE':y.low>x.high?'UP':y.high<x.low?'DOWN':'WITHIN_NOISE';
    return {engine:g.engine,locale:g.locale,market:g.market,previous:p.mentionRate.value,current:g.mentionRate.value,change,modelBreak:stable(p.models)!==stable(g.models)};
  });
  return {comparable:true,groups};
}
...
/* evidenceItem(input): minimal, minimised evidence unit for AI and reports. */
function evidenceItem(v){
  const id=text(v?.id);if(!id)return null;
  return {id,kind:text(v.kind)||'observation',subject:text(v.subject)||null,field:text(v.field)||null,
    value:typeof v.value==='number'||typeof v.value==='boolean'?v.value:(v.value==null?null:minimize(String(v.value),200)),
    text:minimize(v.text,500)||null,url:cleanUrl(v.url)||null,capturedAt:isoOrNull(v.capturedAt),provider:text(v.provider)||null,method:SOURCE_METHODS.includes(v.method)?v.method:'manual'};
}
/* findConflicts(evidence): same subject+field with different values (contradictory). */
function findConflicts(evidence){
  const groups=new Map();
  for(const e of arr(evidence).map(evidenceItem).filter(Boolean)){if(!e.subject||!e.field)continue;const k=e.subject+'|'+e.field;if(!groups.has(k))groups.set(k,[]);groups.get(k).push(e);}
  return [...groups.entries()].filter(([,es])=>new Set(es.map(e=>stable(e.value))).size>1).map(([k,es])=>({group:k,evidenceIds:es.map(e=>e.id).sort()}));
}

/* aiRequest(task, evidence): the structured request handed to an injected model adapter.
   Evidence is whitelisted and minimised; the rules travel with the request. */
function aiRequest(task,evidence){
  if(!AI_TASKS.includes(task))throw new Error('Unknown AI task');
  const items=arr(evidence).map(evidenceItem).filter(Boolean);
  return freeze({schemaVersion:1,task,evidence:items,conflicts:findConflicts(items),
...
const hasPersonalData=s=>/[^\s@<>()"']+@[^\s@<>()"']+\.[a-z]{2,}/i.test(s)||/(?:\+\d[\d\s().-]{7,}\d)|(?:\b\d{3}[\s.-]?\d{3}[\s.-]?\d{3,4}\b)/.test(s)||/\b(api[-_]?key|access[-_]?token|password|passwd|bearer)\b\s*[:=]?\s*\S+/i.test(s)||/https?:\/\/[^\s/]*@/i.test(s);
/* validateAiOutput(output, {task, evidence}): parses tolerant input (object, JSON string,
   fenced JSON) and accepts only structured items whose facts cite known, non-contradictory
   evidence, whose numbers and URLs appear in the cited evidence, with confidence and limits
   and without promises. Output is always a non-canonical suggestion for human review. */
function validateAiOutput(output,{task,evidence}={}){
  if(!AI_TASKS.includes(task))throw new Error('Unknown AI task');
  const base={task,canonical:false,requiresHumanReview:true,accepted:[],rejected:[]};
  let data=output;
  ...
  const items=arr(evidence).map(evidenceItem).filter(Boolean),byId=new Map(items.map(e=>[e.id,e]));
  const conflicted=new Set(findConflicts(items).flatMap(c=>c.evidenceIds));
  ...
  data.items.forEach((it,index)=>{
    ...
    if(kind==='FACT'&&refs.some(r=>conflicted.has(r)))return reject('CONTRADICTORY_EVIDENCE');
    ...
    const cited=refs.map(r=>byId.get(r)),corpus=cited.map(e=>[e.text,e.value].filter(x=>x!=null).join(' ')).join(' ');
    ...
    base.accepted.push({kind,claim,evidenceRefs:refs,evidenceStatus:refs.length?'SUPPORTED':'INSUFFICIENT',confidence:refs.length?it.confidence:'low',limits:minimize(it.limits,300),
      ...(draft?{requiresHumanApproval:true,sendable:false}:{})});
  });
  const status=!base.accepted.length?'REJECTED':base.rejected.length?'PARTIAL':'VALID';
...
/* runAiTask({task, evidence, transport, clock, budget, confirmCost}, {providers}):
   ...
   provenance is recorded and output is redacted. The model adapter is the injected
   transport ({kind, request(operation, input) => {rows:[{output}]}}); none in the Core. */
async function runAiTask(request={},{providers}={}){
  ...
  const input=aiRequest(task,evidence);
  ...
  if(!USABLE.includes(result.status)||!result.data.length)return freeze({task,status:result.status==='EMPTY'?'EMPTY':'NOT_AVAILABLE',providerStatus:result.status,errors:clone(result.errors),provenance:clone(result.provenance||null),canonical:false,accepted:[],rejected:[]});
  const validation=validateAiOutput(result.data[0].output,{task,evidence:input.evidence});
  return freeze({...clone(validation),providerStatus:result.status,provenance:clone(result.provenance),cost:clone(result.cost),budget:clone(result.budget)});
}
...
return Object.freeze({
  ...
  monthlyReport,validateReport,evidenceItem,findConflicts,aiRequest,validateAiOutput,runAiTask,minimize
});
});
