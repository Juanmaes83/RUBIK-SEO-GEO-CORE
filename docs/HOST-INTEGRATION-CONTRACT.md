...
| `src/rubik-seo-geo-platform-contracts.js` (`./platform-contracts`) | `RubikSEOGeoPlatformContracts` | `scope`, `authorize`, `secretRef`, `auditEvent`, `verifyAuditChain`, `spendPolicy`, `spendCheck`, `toProviderBudget`, `recordSpend`, `consentRecord`, `hasConsent`, `signProvenance`, `verifyProvenance`, `createMemoryRepository` (mock), `jobSpec`, `CONNECTORS`, `REPOSITORY_PORTS` (D-25; contratos para la futura plataforma, sin conexiones) |
