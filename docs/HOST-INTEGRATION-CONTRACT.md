...
2. **Reconciliar tras cada cambio** con `core.reconcile(config)` y mostrar `core.preview(config).checks` (PASS/WARNING/ERROR/BLOCKER/OPPORTUNITY/DEFERRED). Debe respetar el modo AUTO/CUSTOM: un campo CUSTOM nunca se sobrescribe.
...
| `src/rubik-seo-geo-release-b.js` (`./content`) | `RubikSEOGeoReleaseB` | Page Registry (`registry`, `page`, `createPage`, `pageContract`, `canPublish`, `migratePath`), blog (`blogArticles`, `validateArticle`, `saveArticle`), enlazado interno (`links`, `brokenLinks`, `orphanPages`), `redirects`, `sitemapEntries`, `audit` |
...
| `src/rubik-seo-geo-intelligence.js` (`./intelligence`) | `RubikSEOGeoIntelligence` | `OpenSEOAdapter`, `SearchConsoleAdapter`, `DataForSEOAdapter` (clientes inyectados), `makeSnapshot`, `diff`, `triage`, `crawlerAudit`, `geoReadiness`, `entityGraph`, `insight` |
| `src/rubik-seo-geo-release-e.js` (`./authority`) | `RubikSEOGeoReleaseE` | E1–E4: `provenance`, `normalizeIndexationRecord`, `normalizePresenceRecord`, `normalizeMentionRecord`, `normalizeCitationObservation`, `indexNowResult`, `record`, `summarize` |
