...
**Única fuente de estado del Core.** Estado actualizado el 25/09/2026: PR #12 (`feat/core-8-offpage-authority`) abierto y no fusionado. Los seis hallazgos de la auditoría están corregidos, con regresiones (D-23, sesión 21). CI de las correcciones: run [36133453921](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/actions/runs/36133453921) del HEAD `d65dcf1` verde en Node 20.20.2/22, 274/274 por job, 0 omitidas. Todo cambio de código y documentación se hace solo en RUBIK-SEO-GEO-CORE. No se modifica ni se consulta WEB-RESTAURACI-N-PREMIUM-DIN-MICA ni ningún otro repositorio. Las lecturas externas requieren autorización expresa y concreta.
...
| **CORE-8** | **SEO off-page & Authority** ([`integrations/OFFPAGE-SERVICE.md`](integrations/OFFPAGE-SERVICE.md)) | 🟡 PR #12 abierto, no fusionado. Los seis hallazgos de la auditoría están corregidos con regresiones que fallan contra `42d213d`: provenance confiable (`providers.isTrustedResult`), cobertura GEO por locale+mercado, rupturas por modelo/superficie/método, conflicto frente a evolución, minimización de todos los campos y validación IA solo estructural (D-23). `npm run verify`: 274 (272 pasan, 2 se omiten en Windows). CI: run [36133453921](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/actions/runs/36133453921) del HEAD `d65dcf1` verde en Node 20.20.2/22, 274/274 por job, 0 omitidas. Queda la revisión humana; no se cierra hasta aprobarla |
| **CORE-8.1** | **Operación off-page continua asistida por IA** ([`integrations/OFFPAGE-SERVICE.md`](integrations/OFFPAGE-SERVICE.md) §6) | 🟡 Implementada en `feat/core-8-1-offpage-operations` ([PR #13](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/pull/13), apilado sobre #12; CI run [36203308792](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/actions/runs/36203308792) del HEAD `a0f59fa` verde en Node 20.20.2/22, 287/287 por job, 0 omitidas; pendiente de revisión humana). Depende de PR #12: fusionar primero CORE-8. Incluye `src/rubik-seo-geo-offpage-ops.js` con los 8 entregables de §6.3: información aprobada, continuidad entre periodos, plan y serie GEO, borradores de contenido y adaptaciones, estudios con datos reales, PR/periodistas/outreach individual, reseñas neutrales e informe periódico. Todo son borradores o propuestas con aprobación humana; sin publicación, envío, modelos ni storage. 13 pruebas en tres verticales; una mutación de 20 guardas mata las 20 (D-24) |
| **CORE-9** | **Platform Layer multi-proyecto (fase final)** ([`core-9/PLATFORM-SPEC.md`](core-9/PLATFORM-SPEC.md)) | 🟡 Solo preparación en este repositorio (D-25), en la rama `docs/core-9-platform-spec` ([PR #14](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/pull/14), apilado sobre #13; CI run [36203720653](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/actions/runs/36203720653) del HEAD `eabd93c` verde en Node 20.20.2/22, 296/296 por job, 0 omitidas; pendiente de revisión humana). Incluye especificación, límites de datos y tenancy, threat model, esquema lógico documental, plan por etapas y contratos con mocks (`src/rubik-seo-geo-platform-contracts.js`: scope/permisos, secretRef, auditoría encadenada, política de gasto → budget de CORE-7, consentimiento, conectores de diseño, provenance firmada, puertos de repositorio con mock y jobSpec). ⛔ **Bloqueada** para implementar hasta responder PLATFORM-SPEC §10: proyecto destino, infraestructura, proveedores y presupuesto, modelo de IA, legal/DPA y primer host. Sin plataforma, secretos, servicios reales, storage ni deploy. El Project State, Studio, Media Library y Page Registry siguen en cada host |

**Secuencia aprobada:** CORE-6 → CORE-7 → CORE-7.1 → corregir/cerrar CORE-8 → CORE-8.1 (operación off-page continua asistida por IA) → CORE-9 (Platform Layer final). CORE-9 puede prepararse aquí con especificación, arquitectura, contratos y mocks; implementar o desplegar la plataforma y activar conexiones reales exige autorización y proyecto destino. CORE-2/4/5 siguen dependiendo de un host; no se inician ni validan.
...
Claude Code debe seguir [`AUTONOMOUS-CONTINUATION.md`](AUTONOMOUS-CONTINUATION.md) y `CLAUDE.md`. Orden obligatorio: (1) resolver y verificar los hallazgos de CORE-8 en PR #12; (2) desarrollar CORE-8.1 en una rama/PR separado que dependa del CORE-8 corregido; (3) avanzar CORE-9 con artefactos revisables dentro de este repositorio hasta el límite de autorización indicado. Antes de pausar por falta de créditos, dejar commit coherente y actualizar HANDOFF con HEAD, pruebas, estado de PRs y siguiente paso exacto. Nunca fusionar, desplegar ni tocar otro repositorio.
