...
| **CORE-7.1** | **Preparación del puente OpenSEO/MCP** ([`integrations/OPENSEO.md`](integrations/OPENSEO.md)) | 🔄 **En progreso** en PR de CORE-7.1 (`feat/core-7-1-openseo-bridge`) (D-22). **Hecho:** contrato Core-only con cliente MCP inyectado para `whoami`, `run_site_audit`, `get_audit_status`, `get_audit_issues` y `get_audit_pages`; solo `structuredContent`; `CONNECTED` solo con health `ok` y `whoami` a través de un cliente live; Lighthouse siempre desactivado y disparador manual; `BRIDGE_PENDING` sin dependencia; mocks deterministas. **Validación:** `npm run verify` da 226 (224 pasan, 2 se omiten en Windows); igual con Node 20.20.2 y 22.23.3 en local. Golden sin cambios. **Falta:** CI del PR, revisión humana y merge. La conexión autenticada y el backend siguen en CORE-9 y `crawl()` sigue siendo el heredado |
