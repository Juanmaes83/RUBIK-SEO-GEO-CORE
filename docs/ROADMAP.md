...
| **CORE-7** | **Preparar contratos de integración para Release E y Release C** | 🔄 **En progreso** en PR de CORE-7 (`feat/core-7-provider-contracts`) (D-21): módulo neutral `providers` (catálogo, sobre de resultado con provenance, estados honestos, errores, datos parciales, confirmación de coste, presupuesto, caché inyectada) y mapeo a los contratos existentes de C y E. Solo mocks; ningún proveedor activado. Revisión aplicada: secretos a cualquier profundidad con redacción ampliada, presupuesto finito obligatorio para quota/paid (`BUDGET_REQUIRED`) y normalización explícita de backlinks. `npm run verify` 201 (199 pasan, 2 se omiten en Windows); Node 20.20.2 y 22.23.3 en local, iguales. Golden sin cambios. Falta: CI del PR, revisión humana y merge |
