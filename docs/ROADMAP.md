...
| **CORE-8** | **SEO off-page & Authority** ([`integrations/OFFPAGE-SERVICE.md`](integrations/OFFPAGE-SERVICE.md)) | 🟡 Implementada en `feat/core-8-offpage-authority`; pendiente de PR, CI y revisión (D-23). Servicio mensual Core-only con perfil y línea base, snapshots con cobertura por fuente y comparación honesta, menciones, NAP, GEO observacional con consultas versionadas e intervalos, oportunidades heurísticas, acciones con aprobación humana, cierre de periodo e informe. La IA solo propone y se valida. Sin storage, conexiones reales, creación de enlaces ni promesas; conectores y modelos reales en CORE-9. `npm run verify` da 262 (260 pasan, 2 se omiten en Windows) |
