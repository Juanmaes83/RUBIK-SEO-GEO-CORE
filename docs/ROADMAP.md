...
**Única fuente de estado del Core.** Estado actualizado el 25/09/2026: PR #12 (`feat/core-8-offpage-authority`) abierto y no fusionado. Los seis hallazgos de la auditoría están corregidos, con regresiones (D-23, sesión 21). CI de las correcciones: pendiente del run de estas correcciones. Todo cambio de código y documentación se hace solo en RUBIK-SEO-GEO-CORE. No se modifica ni se consulta WEB-RESTAURACI-N-PREMIUM-DIN-MICA ni ningún otro repositorio. Las lecturas externas requieren autorización expresa y concreta.
...
| **CORE-8** | **SEO off-page & Authority** ([`integrations/OFFPAGE-SERVICE.md`](integrations/OFFPAGE-SERVICE.md)) | 🟡 PR #12 abierto, no fusionado. Los seis hallazgos de la auditoría están corregidos con regresiones que fallan contra `42d213d`: provenance confiable (`providers.isTrustedResult`), cobertura GEO por locale+mercado, rupturas por modelo/superficie/método, conflicto frente a evolución, minimización de todos los campos y validación IA solo estructural (D-23). `npm run verify`: 274 (272 pasan, 2 se omiten en Windows). CI: pendiente del run de estas correcciones. Queda la revisión humana; no se cierra hasta aprobarla |
