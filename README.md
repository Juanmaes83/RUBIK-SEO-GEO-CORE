...
| Siguiente fase | ✅ CORE-7 cerrado en [PR #10](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/pull/10), merge `6ef8c4e`. Sigue CORE-7.1 (contrato/mock OpenSEO/MCP), CORE-8 (SEO off-page Core-only) y finalmente CORE-9 (Platform Layer e integraciones reales). CORE-2/4/5 siguen sujetos a validación de host. |
...
| Puente OpenSEO/MCP (CORE-7.1) | 🔄 En progreso en PR de CORE-7.1 (`feat/core-7-1-openseo-bridge`): contrato Core-only con cliente MCP inyectado y mocks (D-22). `CONNECTED` solo con health ok y `whoami` confirmado por un verificador inyectado a través de un cliente live; Lighthouse desactivado. La autenticación y el backend reales quedan para CORE-9 |
