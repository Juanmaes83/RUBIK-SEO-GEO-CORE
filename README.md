...
| Puente OpenSEO/MCP (CORE-7.1) | 🔄 En progreso en PR de CORE-7.1 (`feat/core-7-1-openseo-bridge`): contrato Core-only con cliente MCP inyectado y mocks (D-22). `CONNECTED` solo con health ok y `whoami` a través de un cliente live; Lighthouse desactivado. La autenticación y el backend reales quedan para CORE-9 |
