...
| Compatibilidad Restaurant | ✅ golden de `publish()`/`preview()` sin cambios y materialización con 0 diferencias. `core`, `adapters`, `publisher` y `media` con blob idéntico al fuente; `release-b`, `intelligence` y `release-e` derivan desde CORE-3 (D-13…D-15, en revisión) |
...
| Siguiente fase | 🔄 CORE-3 implementado y en revisión en [PR #3](https://github.com/Juanmaes83/RUBIK-SEO-GEO-CORE/pull/3): inyección explícita de Release B, vertical de Release E desde el adapter activo, health de OpenSEO por `/api/health` y validación de rutas en el Page Registry. 113 tests en la rama (111 pasan, 2 se omiten en Windows). CORE-2 sigue bloqueado por alcance (D-12) |
